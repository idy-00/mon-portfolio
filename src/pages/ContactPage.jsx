    ...
    <main>
      ...
      >
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <p>
            Vous avez une idée, un besoin ou vous cherchez un développeur pour renforcer votre équipe ? Je réponds sous 24h.
          </p>
          <div style={{ marginTop: '1rem', fontSize: '0.82rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        <div className="contact-links">
          <a href="mailto:idykane03@gmail.com" className="contact-link">
            <div className="contact-link__icon contact-link__icon--purple">
          ...
          <a href="tel:+221781194805" className="contact-link">
            <div className="contact-link__icon contact-link__icon--cyan">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {step < 3 && (
          <div className="form-steps">
            {STEPS.map((label, i) => (
              <div
                key={label}
                className={`form-step-dot${step === i ? ' active' : ''}${step > i ? ' done' : ''}`}
                title={label}
              >
                {step > i ? (
        ...
        <AnimatePresence mode="wait" custom={dir}>
          ...
          {step === 0 && (
            ...
            >
              ...
              <p className="form-subtitle">Choisissez le formulaire adapté à votre besoin.</p>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '2rem' }}>
                {[
                  { value: 'message', label: 'Message rapide', desc: 'Une question, un bonjour…', icon: '💬' },
                  { value: 'projet', label: 'Demande de projet', desc: 'Brief complet, budget…', icon: '🚀' },
                ].map(opt => (
                  <button
                    key={opt.value}
                    type="button"
                    ...
                    style={{
                      ...
                      border: `1px solid ${form.requestType === opt.value ? 'rgba(124,58,237,0.5)' : 'var(--line)'}`,
                      background: form.requestType === opt.value ? 'rgba(124,58,237,0.1)' : 'var(--glass-1)',
                      color: 'var(--text-main)',
                      cursor: 'pointer',
                      textAlign: 'left',
                      transition: 'all 0.2s',
                  ...
                  >
                    ...
                    <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, marginBottom: '0.25rem' }}>{opt.label}</div>
              ...
              <div className="form-actions" style={{ justifyContent: 'flex-end' }}>
                <motion.button type="button" className="btn btn--primary" onClick={goNext} whileTap={{ scale: 0.97 }}>
                  Continuer
          ...
          {step === 1 && (
            ...
            >
              <p className="form-title">
                {form.requestType === 'message' ? 'Votre message' : 'Votre projet'}
              </p>
              ...
              {form.requestType === 'message' ? (
                ...
                <>
                  <div className="form-row">
                    <div className="form-field">
                      <label className="form-label" htmlFor="company">Entreprise / Client</label>
                    ...
                    <div className="form-field">
                      ...
                      <input id="budget" className="form-input" type="number" min="0" value={form.budget}
                        onChange={e => set('budget', e.target.value)} placeholder="Ex: 1500" />
                    </div>
                  </div>
                  <div className="form-field">
                  ...
                  <div className="form-field">
                    <label className="form-label" htmlFor="details">Description du besoin *</label>
          ...
          {step === 2 && (
            ...
            >
              ...
              <p className="form-subtitle">Vérifiez vos informations avant d'envoyer.</p>

              <div style={{
                background: 'var(--glass-2)', border: '1px solid var(--line-medium)',
                ...
                {[
                  ...
                  form.requestType === 'message'
                    ...
                    : ['Projet', `${form.projectType} — ${form.company || 'N/A'} — ${form.budget ? form.budget + ' €' : 'Budget non précisé'}`],
                ].map(([label, value]) => (
                  <div key={label} style={{ display: 'flex', gap: '1rem', fontSize: '0.875rem' }}>
                    <span style={{ color: 'var(--text-muted)', minWidth: 60, fontWeight: 600 }}>{label}</span>
                    <span style={{ color: 'var(--text-main)', flex: 1, wordBreak: 'break-word' }}>{value || '—'}</span>
                  </div>
                ))}
              </div>

              {status === 'error' && (
                <div className="form-feedback form-feedback--error" style={{ marginBottom: '1rem' }}>
                  Une erreur est survenue. Réessayez ou écrivez directement à idykane03@gmail.com.
                </div>
              )}
          ...
          {step === 3 && (
            ...
            >
              ...
              <p style={{ color: 'var(--text-sub)', marginBottom: '2rem', lineHeight: 1.7 }}>
                Merci {form.name ? form.name.split(' ')[0] : ''} ! Je vous répondrai dans les 24h.<br />
                En attendant, retrouvez-moi sur LinkedIn ou GitHub.
              </p>
              <button className="btn btn--outline" onClick={reset}>Envoyer un autre message</button>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
